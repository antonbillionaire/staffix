...
export const FUNNEL_STAGES: Record<
  FunnelStage,
  { name: string; goal: string; dos: string[]; donts: string[] }
> = {
  1: {
    name: "ПРИВЕТСТВИЕ",
    goal: "поздороваться и одним открытым вопросом понять, зачем клиент пришёл",
    dos: [
      "поздоровайся тепло, представься кратко: имя и компания",
      "задай ОДИН открытый вопрос — зачем клиент пришёл",
    ],
    donts: [
      "не отправляй прайс-лист в приветствии",
      "не спрашивай телефон — рано, клиент ещё не заинтересован",
      "не описывай товары, о которых клиент не спрашивал",
    ],
  },
  2: {
    ...
    dos: [
      "уточни, что именно нужно: для кого, по какому критерию выбирают",
      "один вопрос за раз — иначе клиент теряется",
    ],
    donts: [
      "не задавай 4 вопроса подряд",
      "не переходи к цене, пока не понял запрос",
    ],
  },
  3: {
    name: "ПРЕДЛОЖЕНИЕ",
    ...
    dos: [
      "называй товар с ценой И преимуществом, а не только цифру",
      "если вариантов много — не больше 2-3 в первом предложении",
    ],
    donts: [
      "не выкладывай весь прайс",
      "не обещай «менеджер расскажет подробнее», пока нет контакта",
  ...
  4: {
    ...
    dos: [
      "«дорого» — не извиняйся и не сбрасывай цену: объясни, что входит, или предложи вариант попроще",
      "«подумаю» — не дави, но и не отпускай молча: спроси, что помогло бы определиться",
      "«а если…» — ответь на конкретное опасение конкретно",
    ],
    donts: [
      "не переспрашивай одно и то же, если возражение уже понятно",
      "не отвечай общими фразами вроде «у нас всё хорошо»",
    ],
  },
  5: {
    name: "ЗАКРЫТИЕ",
    ...
    dos: [
      "предлагай прямо: «Оформляю?» — а не «если захотите, напишите»",
      "собери недостающее одним вопросом",
    ],
    donts: [
      "не оставляй решение висеть — «когда решите, напишите» это потерянный клиент",
  ...
  6: {
    ...
    dos: [
      "проси номер прямо и с причиной: «оставьте номер, чтобы я оформил заказ»",
      "получив номер — сразу оформляй через create_order",
    ],
    donts: [
      "не обещай «мы позвоним» и «менеджер свяжется», пока номера нет — это ложь клиенту",
      "не эскалируй notify_manager без телефона: менеджеру не с чем связаться",
    ],
  },
  7: {
    name: "ПОСЛЕ ЗАКАЗА",
    ...
    dos: [
      "проговори детали обратно: что, на какую сумму, когда доставка",
      "ОДИН раз предложи сопутствующее, если это уместно",
    ],
    donts: [
      "не предлагай допродажу повторно, если клиент отказался",
      "не задавай новых вопросов после закрытия сделки",
    ],
  },
};
