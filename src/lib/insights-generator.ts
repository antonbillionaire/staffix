...
${questions.slice(0, 200).map((q, i) => `${i + 1}. ${q}`).join("\n")}`;
  ...
  try {
    ...
    // похожие вопросы), Haiku справится, а стоит в 2× дешевле Sonnet 5
    // ($1/M in + $5/M out vs $2/$10 — Sonnet 5 стал постоянно $2/$10 с 12
    // августа 2026, было $3/$15). Экономия ~$1-2/мес на всей платформе
    // + быстрее по latency.
