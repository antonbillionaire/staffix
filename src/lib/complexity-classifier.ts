...
/**
 * Узбекский / казахский / киргизский в сообщении клиента.
 *
 * Латиница: слова, которых нет ни в русском, ни в английском. Кириллица:
 * буквы ў, қ, ғ, ҳ, ә, ң, ө, ү, һ — их нет в русском алфавите, поэтому одной
 * такой буквы достаточно; плюс частотные слова, которые пишут и без них.
 *
 * Специально широкий и дешёвый: цена ошибки в одну сторону — лишний Sonnet
 * на русском сообщении, в другую — выдуманный ассортимент на узбекском.
 */
export function isNonSlavicLanguage(text: string): boolean {
  if (/[ўқғҳәңөүһЎҚҒҲӘҢӨҮҺ]/.test(text)) return true;
  if (/(сизга|сизнинг|борми|қанча|канча|рахмат|салом|учун|керак|яхши|нима|бўлади|булади)/i.test(text)) {
    return true;
  }
  return /\b(salom|assalom|rahmat|bormi|qancha|narxi|sizga|kerak|yaxshi|mumkinmi|nima|qanday|uchun|bo['’`ʻ]?ladi|men\s+xohlayman)\b/i.test(
    text
  );
}
...
export async function classifyMessageComplexity(
  ...
  // Сообщение не на русском и не на английском — всегда Sonnet (17 сент 2026).
  //
  // Замерено на эвалах: те же узбекские кейсы Sonnet проходит, Haiku валит
  // оба. На вопрос «Salom! BB-krem bormi? Narxi qancha?» Haiku выдумал
  // несуществующие варианты товара (Light / Medium вместо реальных тонов
  // 01 / 02), не назвал цену и написал ломаным узбекским
  // («qaysi oynali turga mos kelmini»). Это не вопрос стиля: выдуманный
  // ассортимент — прямое нарушение правила «AI не выдумывает факты».
  //
  // Узбекский и казахский — основные языки целевого рынка, и на них
  // приходится четверть ответов бота у OLLEE. Экономия на них не окупается.
  if (isNonSlavicLanguage(trimmed)) return "complex";
