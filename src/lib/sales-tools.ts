...
export const salesToolDefinitions: Anthropic.Tool[] = [
  ...
  },
  {
    name: "advance_stage",
    description:
      "Сообщить, что разговор перешёл на следующую стадию воронки. Вызывай КОГДА цель текущей стадии достигнута: клиент назвал что хочет (стадия 2), ты сделал предложение (3), клиент возразил (4), клиент согласился покупать (5), клиент дал контакт (6), заказ оформлен (7). Оставаться на текущей стадии можно молча — для этого инструмент не нужен. Переход проверяет система: назад пустит только на стадию возражений, а на 6-7 — только когда у клиента есть телефон.",
    input_schema: {
      type: "object" as const,
      properties: {
        stage: {
          type: "integer",
          description:
            "Номер новой стадии: 1 приветствие, 2 выяснение потребности, 3 предложение, 4 возражения, 5 закрытие, 6 получение контакта, 7 после заказа",
        },
        reason: {
          type: "string",
          description: "Коротко, что именно произошло в разговоре — одна фраза",
        },
      },
      required: ["stage"],
    },
...
export async function executeSalesTool(
  ...
  try {
    switch (toolName) {
      // advance_stage ничего не делает здесь намеренно: стадию записывает
      // вызывающий код после оборота, когда известны и факты диалога, и
      // результат валидации перехода (funnel-state.ts). Инструмент нужен
      // только как способ для модели СООБЩИТЬ о переходе — решение за кодом.
      case "advance_stage":
