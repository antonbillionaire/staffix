    ...
    input_schema: {
      ...
      properties: {
        ...
        staff_id: {
          type: "string",
          description:
            "ID мастера/специалиста ОБЯЗАТЕЛЬНО когда в бизнесе больше одного мастера и понятно к кому записываемся. Определи его так: (1) если клиент назвал имя мастера — найди ID в списке staff. (2) если клиент назвал специализацию ('хочу к терапевту', 'к кардиологу') — найди мастера с этой ролью. (3) если услуга подразумевает конкретного специалиста (например, 'Терапевт первичный приём' → мастер с ролью 'терапевт') — выбери его. Только если в бизнесе ОДИН мастер ИЛИ клиент явно сказал 'к любому' — можно не передавать.",
        },
        client_phone: {
          type: "string",
