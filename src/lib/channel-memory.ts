...
export function buildClientContextBlock(context: ChannelClientContext): string {
  ...
  //
  // Баг, который блокировал заказы: телефон СОБИРАЛСЯ (60 шт за месяц) и
  // сохранялся в ChannelClient.phone, тип ChannelClientContext его грузил —
  // но в текст промпта он не попадал. Модель каждый оборот считала, что
  // телефона нет, а описание инструмента create_order требует «ОБЯЗАТЕЛЬНО
  // уточни телефон» → инструмент не вызывался → бот обещал передать
  // менеджеру → handoff-guard ловил обещание → эскалация.
  // Итог за 30 дней: 10 собранных заказов в эскалациях, создан 1.
  //
  // Правило владельца: телефон для заказа обязателен всегда. Поэтому когда
  // телефона нет — пишем это явно, чтобы модель знала, что его надо получить,
  // а не догадывалась по отсутствию строки.
  if (context.phone) {
    block += `\n- Телефон: ${context.phone} (УЖЕ ПОЛУЧЕН — не переспрашивай, используй при оформлении заказа)`;
  } else {
