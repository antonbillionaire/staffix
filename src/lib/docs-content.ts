...
**Email-рассылки** — Рассылка не только в Telegram, но и на email. Resend + одноклик-отписка по требованию закона.
...
**WhatsApp Business:**
1. \`/dashboard/channels\` → WhatsApp → «Подключить через Facebook»
2. Откроется окно Embedded Signup — войдите в Facebook (нужен бизнес-аккаунт Meta)
3. Выберите или создайте Meta Business Account
4. Выберите или зарегистрируйте номер WhatsApp Business — Meta проведёт по всем шагам, включая верификацию номера
5. Подтвердите права Staffix (Tech Provider)

**Instagram и Facebook:**
1. \`/dashboard/channels\` → Instagram или Facebook → «Подключить через Facebook»
2. Выберите страницу Facebook (Instagram Business Account должен быть привязан к странице)
3. Подтвердите разрешения на сообщения и комментарии
...
3. Выберите или зарегистрируйте номер WhatsApp Business через Embedded Signup — Meta проведёт по шагам

**Instagram и Facebook:**
1. \`/dashboard/channels\` → Instagram или Facebook
2. Подключите через Facebook OAuth, выберите страницу (для IG — с привязанным Business Account)
...
**Что умеет Telegram ИИ сотрудник:**
- Отвечает на вопросы 24/7 с использованием AI
- Записывает клиентов (проверяет расписание, предотвращает конфликты)
- Принимает заказы на товары
- Отправляет напоминания за 24ч и 2ч, собирает отзывы, реактивирует «спящих»
- Продвигает клиента по воронке продаж (dealStage) и ставит задачи менеджеру
- Понимает голосовые сообщения (Groq Whisper, ru/uz/kz/en)
- Использует базу знаний, FAQ, каталог и AI Memory каждого клиента
- Начисляет и списывает баллы программы лояльности

**Как подключить:**
1. Создайте бота через @BotFather → /newbot
2. Скопируйте токен (вида 123456789:ABCdef...)
3. \`/dashboard/channels/telegram\` → вставьте токен → «Активировать»

### WhatsApp Business

Подключено и работает через Meta Business Cloud API (Embedded Signup).

**Как подключить:**
1. \`/dashboard/channels\` → WhatsApp → «Подключить»
2. Появится окно Facebook Login for Business
3. Выберите Meta Business Account (или создайте новый в этом же окне)
4. Выберите или зарегистрируйте номер WhatsApp Business
5. Согласуйте разрешения Staffix (Tech Provider)

После подключения ИИ сотрудник отвечает в WhatsApp точно так же как в Telegram — та же база знаний, тот же AI Memory, те же tools (запись, заказы, лояльность).

**Ограничения WhatsApp:**
- Meta требует бизнес-номер (не личный)
- Первое сообщение клиенту вне 24-часового окна — только через утверждённый шаблон
- Медиа-сообщения от клиента (фото, файлы): AI ответит текстом с просьбой описать словами

### Instagram Direct

Подключено и работает — @staffixio уже отвечает через Staffix.

**Как подключить:**
1. Ваша FB-страница должна быть связана с Instagram Business Account
2. \`/dashboard/channels\` → Instagram → «Подключить» (через Facebook Login)
3. Выберите Instagram Business Account
4. Подтвердите права \`instagram_manage_messages\` + \`instagram_manage_comments\`

**Что умеет:**
- Отвечает в Direct 24/7 (все функции TG-бота)
- Отвечает на комментарии под постами и рекламой через приватное сообщение (Comment-to-DM)
- Работает с ad-referral (знает из какой рекламы пришёл клиент — заполняется \`Lead.adId\`)

### Facebook Messenger

Подключено и работает через Meta Cloud API.

**Как подключить:**
1. \`/dashboard/channels\` → Facebook → «Подключить» (Facebook Login)
2. Выберите Facebook Page бизнеса
3. Подтвердите права \`pages_messaging\`

ИИ сотрудник отвечает в Messenger так же как во всех остальных каналах.

### Единая база клиентов

Все клиенты из всех каналов попадают в единую CRM Staffix. Независимо от канала (TG/WA/IG/FB/сайт), история хранится в \`/dashboard/customers\`, диалоги — в \`/dashboard/messages\`.`,
...
3. В Staffix: \`/dashboard/payments\` → раздел Payme
...
3. В Staffix: \`/dashboard/payments\` → раздел Click
...
**Где:** \`/dashboard/business\` → «Загрузить логотип»
...
1. \`/dashboard/ads\` → блок «Рекламный аккаунт»
2. Введите Ad Account ID из Meta Ads Manager (например \`123456789\`, без префикса \`act_\`)
3. Токен доступа берётся из уже подключённого Facebook Login (\`/dashboard/channels\`). Если Meta-каналы работают — токен уже есть.
4. Данные появятся в течение суток (крон раз в день в 07:00 UTC), либо нажмите «Обновить» на странице \`/dashboard/ads\` для ручной синхронизации.

### Что вы видите на странице \`/dashboard/ads\`
...
Менеджерден қолмен: сан + пікір → «Есептеу» немесе «Алу». Шектеу ±100 000. Теріс теңгерім мүмкін емес.`,
  ...
  // ===== 30. WIDGET (Sprint Widget — 21 июля 2026) =====
  {
    ...
    description: {
      ru: "Плавающая кнопка → AI-чат прямо на сайте + мессенджеры",
      en: "Floating button → AI chat right on the site + messenger buttons",
      uz: "Suzuvchi tugma → saytda AI chat + messenjerlar",
      kz: "Қалқымалы түйме → сайттағы AI чат + мессенджерлер",
    },
    content: {
      ru: `## Виджет для сайта

Разместите на своём сайте плавающую кнопку — посетитель кликает и **общается с вашим AI-сотрудником прямо на сайте**. Внизу окна — кнопки Telegram / WhatsApp / Instagram / Messenger для тех, кто предпочитает мессенджер.
...
1. Круглая кнопка справа-снизу (цвет и иконка — из ваших настроек).
2. Клик → раскрывается окно чата с приветствием: *«Здравствуйте! Чем могу помочь?»*
3. Пишет сообщение → бот отвечает через 2-4 секунды. Тот же AI-сотрудник, что в мессенджерах: знает услуги, товары, FAQ; умеет записывать / оформлять заказы; передаст менеджеру если нужно.
4. Внизу окна — маленькие иконки TG / WA / IG / Messenger. Клик → откроется мессенджер с готовым чатом.
5. Диалог сохраняется в браузере посетителя — если он вернётся, продолжит с того же места.
...
Веб-сообщения считаются как обычные — входят в лимит вашего тарифа:
- **Pro** ($45/мес): 1000 сообщений включая веб
- **Business** ($95/мес): 3000 сообщений

Никакой отдельной подписки для веб-чата нет.

### Управление

- **Подключили новый канал** — иконка появится в виджете автоматически через ~5 минут (edge-cache).
- **Отключили канал** — иконка пропадёт у посетителей.
- **Бот на паузе** (тумблер в дашборде) — виджет показывает сообщение «чат временно недоступен, напишите в мессенджер» и кнопки TG/WA/IG.
...
Веб-разговоры появляются в **Клиенты** как обычные с бейджем «web». Работают:
- deal-pipeline (лид → консультация → клиент)
- уведомления менеджеру (notify_manager)
- лояльность (начисляет баллы через все каналы одинаково)
- AI-инсайты (веб-диалоги учитываются в еженедельном отчёте)

Веб-визитор идентифицируется через **visitor_id** в localStorage браузера — стабильный за 30 дней. Если он даст телефон в чате — совпадёт с существующим клиентом (если есть).

### Защита от спама

Автоматически:
- **Rate-limit 30 сообщений/час** с одного IP посетителя
- **Rate-limit 300 сообщений/час** на один бизнес (защита от DDoS на конкретный сайт)
- **Honeypot** — скрытое поле в форме, боты его заполняют, люди — нет
- **Валидация**: сообщение не длиннее 1000 символов, visitor_id только буквы/цифры

Ручных настроек не нужно, всё работает из коробки.

### Технические детали (для тех кому интересно)

- Скрипт грузится асинхронно (\`async\`), не блокирует рендеринг сайта.
- Размер ~10 КБ (без React, чистый vanilla JS + chat UI).
- localStorage для visitor_id и последних 20 сообщений (континуитет диалога).
- CORS \`*\` — работает на любом домене.
- Если у вашего сайта строгий CSP — добавьте \`staffix.io\` в \`script-src\` и \`connect-src\`.`,
      en: `## Website Widget
...
**WhatsApp үшін**: профильде телефон халықаралық форматта көрсетілуі керек (+998...).`,
  ...
  },
  {
    ...
    title: {
      ...
      en: "Loyalty program",
      uz: "Sadoqat dasturi",
      kz: "Адалдық бағдарламасы",
    },
    description: {
      ru: "Уровни клиентов, кэшбек, автоматическое повышение",
      en: "Tiers, cashback, auto-promotion",
      uz: "Darajalar, keshbek, avtomatik yuksalish",
      kz: "Деңгейлер, кэшбэк, автоматты жоғарылату",
    },
    content: {
      ru: `## Программа лояльности

Клиенты копят кэшбек на будущие покупки/визиты — ИИ сотрудник сам считает баллы и учитывает уровень при разговоре.

### Как это работает

1. У бизнеса есть до 4 уровней: **Bronze / Silver / Gold / Platinum**. Каждому уровню задаётся процент кэшбека (например 3% / 5% / 7% / 10%).
2. После каждой оплаченной записи (Booking) или заказа (Order) клиент получает **баллы = сумма × % его уровня**. 1 балл = 1 рубль/сум/тенге (валюта бизнеса).
3. При следующей покупке ИИ сотрудник предлагает применить баллы — клиент отвечает «да, спишите N баллов», сумма к оплате уменьшается.
4. Клиент автоматически переходит на следующий уровень при достижении порога (например 5 визитов = Silver, 15 = Gold, 30 = Platinum).

### Настройка — \`/dashboard/loyalty\`

- **Уровни**: активируйте нужное количество (можно только Bronze, можно все 4)
- **Кэшбек по уровню**: процент от каждой покупки, который вернётся баллами
- **Пороги повышения**: сколько визитов/заказов/потраченной суммы нужно для следующего уровня
- **Срок жизни баллов**: например «сгорают через 6 месяцев без активности» (или без ограничения)
- **Минимальная сумма для списания**: например «списать баллы можно от 500»

### Что видит клиент в переписке

- «Ваш уровень: Silver. Кэшбек 5%.»
- «У вас 1 240 баллов. За эту запись начислим +150. Хотите списать баллы сейчас?»
- «Списываю 500 баллов, к оплате: 2 500 сум.»

### История начислений

Все изменения баланса пишутся в **LoyaltyLedger** — по каждому клиенту видно за что начислили/списали и когда. Прямых правок баланса в БД **нет** — только через транзакцию LoyaltyLedger (защита от рассинхрона).

### Ограничения

- Программа лояльности — на весь бизнес, не по услугам/товарам. Если хотите разные % на разные товары — используйте скидки, не лояльность.
- Работает во всех каналах одинаково (TG/WA/IG/FB/сайт) — баланс общий по клиенту.`,
      en: `## Loyalty program

Clients earn cashback on future purchases/visits — the AI employee counts points and factors the tier into conversations.

### How it works

1. Up to 4 tiers: Bronze / Silver / Gold / Platinum, each with its own cashback %.
2. After every paid booking or order the client earns points = amount × their tier %.
3. Next purchase: the AI offers to redeem points — the amount to pay decreases.
4. Auto-promotion when the client hits the tier threshold (5 visits → Silver, etc).

### Setup — \`/dashboard/loyalty\`

Configure active tiers, cashback %, promotion thresholds, point expiration, and the min amount for redemption.

All balance changes are written to **LoyaltyLedger** — full history per client. Direct DB writes are forbidden (transaction integrity).`,
      uz: `## Sadoqat dasturi

Mijozlar keyingi xaridlar uchun keshbek to'playdi. Sozlash: \`/dashboard/loyalty\`. 4 darajagacha (Bronze/Silver/Gold/Platinum), avtomatik yuksalish.`,
      kz: `## Адалдық бағдарламасы

Клиенттер болашақ сатып алулар үшін кэшбэк жинайды. Баптау: \`/dashboard/loyalty\`. 4 деңгейге дейін, автоматты жоғарылату.`,
    },
  },
  {
    id: "partner-program",
    icon: "Users2",
    title: {
      ru: "Партнёрская программа Staffix",
      en: "Staffix partner program",
      uz: "Staffix hamkorlik dasturi",
      kz: "Staffix серіктестік бағдарламасы",
    },
    description: {
      ru: "Рекомендуйте Staffix — получайте 20% с каждой оплаты",
      en: "Refer Staffix — earn 20% of every payment",
      uz: "Staffix'ni tavsiya qiling — har to'lovdan 20%",
      kz: "Staffix ұсыныңыз — әр төлемнен 20%",
    },
    content: {
      ru: `## Партнёрская программа Staffix

Если вы уже пользуетесь Staffix и хотите рекомендовать нас другим бизнесам — становитесь партнёром. За каждого клиента, пришедшего по вашей ссылке, вы получаете **20%** с каждой его оплаты.

### Как стать партнёром

1. Зайдите на \`/partner/dashboard\` (или через кнопку «Партнёрская программа» в дашборде)
2. Получите свою уникальную реферальную ссылку вида \`https://staffix.io/?ref=YOUR_CODE\`
3. Делитесь ссылкой с бизнесами, которым может пригодиться ИИ сотрудник

### Как работает атрибуция

- Клиент переходит по вашей ссылке → в его браузере ставится cookie **на 60 дней**
- Если клиент зарегистрируется в течение 60 дней — он привяжется к вам
- Клиент оплачивает подписку → вы автоматически получаете **20%** (комиссия по умолчанию — при спец-договорённостях бывает выше)

### Ваш дашборд \`/partner/dashboard\`

- Список ваших рефералов и их статус подписки
- Заработано всего / выплачено / в холде
- История выплат
- Реферальная ссылка + промо-материалы

### Выплаты

- Партнёрские начисления сначала попадают в **холд** (защита от возвратов)
- После истечения периода холда (обычно 30 дней) начисления автоматически разблокируются — cron \`/api/cron/partner-earnings-unlock\` каждый день в 06:00 UTC
- Выплата — по запросу через дашборд

### Ограничения

- Собственные бизнесы не считаются (нельзя зарегистрироваться по своей же ссылке)
- Комиссия идёт только с подписки (не с сумм, которые клиент зарабатывает через Staffix)
- Партнёр может видеть только агрегатную статистику своих рефералов, не их клиентов`,
      en: `## Staffix partner program

Refer other businesses to Staffix — earn 20% of every payment they make.

- Get your referral link at \`/partner/dashboard\`
- 60-day cookie for attribution
- Auto-tracking of your referrals and earnings
- Payouts on request after 30-day holdback (protection from chargebacks)`,
      uz: `## Staffix hamkorlik dasturi

Staffix'ni tavsiya qiling — har bir referal to'lovidan 20% oling. Havolangizni \`/partner/dashboard\`'dan oling. 60 kunlik cookie.`,
      kz: `## Staffix серіктестік бағдарламасы

Staffix ұсыныңыз — әрбір рефералдың төлемінен 20% алыңыз. Сілтемені \`/partner/dashboard\`-тан алыңыз. 60 күндік cookie.`,
    },
  },
  {
    ...
    icon: "Radio",
    title: {
      ru: "Кастомизация виджета для сайта",
      en: "Site widget customization",
      uz: "Sayt vidjetini moslashtirish",
      kz: "Сайт виджетін теңшеу",
    },
    description: {
      ru: "Цвет, аватар, приветственное сообщение, заголовок",
      en: "Color, avatar, greeting, header",
      uz: "Rang, avatar, salomlashish, sarlavha",
      kz: "Түс, аватар, сәлемдесу, тақырып",
    },
    content: {
      ru: `## Кастомизация виджета

Веб-виджет из \`/dashboard/widget\` можно подогнать под ваш бренд — цвет, иконку, приветствие, заголовок.

### Что можно настроить

- **Цвет** (\`widgetColor\`) — основной цвет плавающей кнопки и шапки чата. Задаётся HEX (например \`#4f46e5\`).
- **Иконка/аватар** (\`widgetIcon\`) — картинка внутри кнопки. Можно загрузить логотип бизнеса (Vercel Blob) или использовать emoji.
- **Приветственное сообщение** (\`widgetGreeting\`) — первое сообщение от ИИ сотрудника, которое видит посетитель сайта. Например: «Здравствуйте! Я помощник клиники "Ромашка". Чем могу помочь?»
- **Заголовок** (\`widgetTitle\`) — текст в шапке чата, например «Онлайн-запись» или «Служба поддержки».

### Где настроить

\`/dashboard/widget\` → раздел «Внешний вид»:
- Цвет — color-picker
- Иконка — загрузить файл или вставить URL
- Приветствие — многострочный текст
- Заголовок — однострочный текст

Изменения применяются к виджету на сайте в течение ~5 минут (cache).

### Что виджет умеет по функционалу

- Полноценный AI-чат: то же самое что бот в мессенджерах — база знаний, каталог, запись, лояльность
- Клиент вводит имя и/или телефон (опционально) — все обращения попадают в \`/dashboard/customers\`
- История диалога сохраняется в браузере посетителя (localStorage) — если он вернётся на сайт, чат продолжится
- Rate-limit fail-open — если что-то идёт не так, виджет молча деградирует, не ломает ваш сайт

### Установка

Скопируйте код на \`/dashboard/widget\` и вставьте перед \`</body>\` на всех страницах сайта. Инструкции для WordPress/Tilda/Wix — там же.`,
      en: `## Widget customization

Customize the site widget from \`/dashboard/widget\`:
- **Color** — the floating button and chat header color (HEX)
- **Icon/avatar** — image inside the button (Vercel Blob upload or emoji)
- **Greeting** — the first message from the AI employee shown to visitors
- **Title** — chat header text

Changes propagate to the live widget within ~5 min.`,
      uz: `## Vidjet moslashtirish

\`/dashboard/widget\` orqali rang, avatar, salomlashish va sarlavhani sozlang. Vidjet 5 daqiqada yangilanadi.`,
      kz: `## Виджетті теңшеу

\`/dashboard/widget\` арқылы түс, аватар, сәлемдесу және тақырыпты баптаңыз. 5 минутта жаңарады.`,
    },
  },
  {
    id: "voice-messages",
    ...
    title: {
      ...
      en: "Voice messages",
      uz: "Ovozli xabarlar",
      kz: "Дауыстық хабарламалар",
    },
    description: {
      ru: "ИИ сотрудник понимает голосовые в TG/WA/IG/FB",
      en: "AI employee understands voice messages across all channels",
      uz: "AI-xodim barcha kanallarda ovozli xabarlarni tushunadi",
      kz: "AI-қызметкер барлық арналарда дауыстық хабарламаларды түсінеді",
    },
    content: {
      ru: `## Голосовые сообщения

ИИ сотрудник понимает голосовые сообщения во всех каналах — Telegram, WhatsApp, Instagram Direct, Facebook Messenger. Клиенту не нужно писать текст — можно надиктовать вопрос голосом, ИИ распознает и ответит.

### Как это работает

1. Клиент присылает voice-note в мессенджер
2. Staffix скачивает аудио через API канала
3. Отправляет в **Groq Whisper** (speech-to-text)
4. Распознанный текст передаётся ИИ сотруднику как обычное сообщение
5. Ответ уходит клиенту **текстом** (голосом обратно пока не отвечаем)

### Языки

Whisper распознаёт **русский, узбекский, казахский, английский** — те же 4 языка что и текстовый AI. Определение языка автоматическое.

### Ограничения

- Максимальная длина голосового: обычно до 5 минут (лимиты Whisper)
- Плохое качество записи (шум, тихий голос) → распознавание может быть неточным
- Ответ всегда текстовый — text-to-speech (голос обратно) пока не делаем
- Файлы (не голос, а аудио-файлы) не поддерживаются в Instagram, работают в TG/WA

### Настройка

Ничего настраивать не нужно — голосовые работают из коробки, если бот подключён к каналу.`,
      en: `## Voice messages

The AI employee understands voice notes in Telegram, WhatsApp, Instagram Direct, and Facebook Messenger.

- Groq Whisper for speech-to-text (Russian, Uzbek, Kazakh, English — auto-detected)
- Reply is always text (no TTS back yet)
- Works out of the box — no setup needed`,
      uz: `## Ovozli xabarlar

AI-xodim TG/WA/IG/FB'da ovozli xabarlarni tushunadi (Groq Whisper). 4 til: ru/uz/kz/en. Javob har doim matnda.`,
      kz: `## Дауыстық хабарламалар

AI-қызметкер TG/WA/IG/FB-та дауыстық хабарламаларды түсінеді (Groq Whisper). 4 тіл. Жауап әрқашан мәтінде.`,
    },
  },
  {
    id: "ai-helper",
    icon: "Brain",
    title: {
      ru: "AI-помощник в дашборде",
      en: "In-dashboard AI helper",
      uz: "Boshqaruv panelidagi AI-yordamchi",
      kz: "Панельдегі AI-көмекші",
    },
    description: {
      ru: "Плавающий чат-помощник по функциям Staffix — отвечает голосом Staffix",
      en: "Floating in-dashboard assistant that answers Staffix questions",
      uz: "Staffix haqida savollarga javob beruvchi vidjet",
      kz: "Staffix туралы сұрақтарға жауап беретін виджет",
    },
    content: {
      ru: `## AI-помощник в дашборде

В правом нижнем углу каждой страницы дашборда есть плавающая кнопка **«Помощник Staffix»** (градиент синий→фиолетовый, иконка искры). Нажмите — откроется чат.

### Что он умеет

Помощник знает всю документацию Staffix и текущее состояние вашего бизнеса. Задавайте вопросы естественным языком:
- «Как настроить WhatsApp?»
- «Где посмотреть статистику?»
- «Как загрузить прайс в базу знаний?»
- «Как сделать рассылку?»
- «Почему бот не отвечает клиентам?»

### Что делает его умнее обычного чата

- **Знает что уже настроено**: сколько услуг/товаров/каналов у вас есть. Если нет каналов — приоритетно скажет «сначала подключите /dashboard/channels».
- **Знает где вы находитесь**: если вы задаёте вопрос на странице \`/dashboard/knowledge\`, ответ будет в контексте базы знаний.
- **Кликабельные ссылки**: любая ссылка на страницу дашборда в ответе — сразу переход одним кликом.
- **История сохраняется в браузере**: закрыли вкладку — вернулись, диалог продолжается (последние 20 сообщений).

### Бесплатно

Помощник входит в подписку — за вопросы к нему не платите отдельно. Расходы на AI покрываются нами как часть Staffix.

### Ограничения

- Rate-limit 60 сообщений в час — на случай если помощник зациклится, не блок для реальной работы
- Отвечает только про Staffix — не заменяет ChatGPT для общих вопросов
- Если помощник не уверен — скажет «посмотрите /docs» вместо выдумывания

### Управление

В шапке чата: **«Сброс»** очищает историю. **«×»** сворачивает. **«Отключить»** в футере — если помощник вам не нужен, не будет больше показываться на страницах дашборда (можно вернуть через настройки браузера — очистить localStorage).`,
      en: `## In-dashboard AI helper

Floating button at the bottom-right of every dashboard page — click to open the chat.

Ask natural-language questions about Staffix ("how do I set up WhatsApp?"), get answers with clickable links to the right dashboard page. Knows your current business state (channels, catalog, knowledge base) and prioritises setup steps you're missing.

Free — included in your subscription. History persists in browser localStorage.`,
      uz: `## Panelda AI-yordamchi

Har bir sahifaning o'ng pastki burchagida suzuvchi tugma. Staffix haqida savollar bering — javob va kerakli sahifaga havola. Bepul.`,
      kz: `## Панельдегі AI-көмекші

Әр беттің оң жақ төменгі бұрышында қалқымалы түйме. Staffix туралы сұрақтар қойыңыз — жауап пен қажетті бетке сілтеме. Тегін.`,
    },
  },
];
